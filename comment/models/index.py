...
class Product(db.Model):
    ...
    __table_args_ = {'comment':'商品表'}

    id = db.Column(db.BigInteger, primary_key=True, comment='商品id')
    ...
    album_pics = db.Column(db.Text, comment='商品组图,加上默认主图，最多允许5张图，逗号 分割')
    sales_num = db.Column(db.Integer, comment='销量')
    detail_desc = db.Column(db.String(512), comment='商品详情描述')
    publish_status = db.Column(db.SmallInteger, comment='上架状态：0->下架；1->上 架')
    detail_html = db.Column(db.String(512), comment='商品详情web端页面，基本以图为主， 富文本HTML样式')
    enabled = db.Column(db.SmallInteger, nullable=False, comment='逻辑删除 0-未删 除，1-删除')
...
class HomeNewProduct(db.Model):
    ...
    gmt_create = db.Column(db.BigInteger, nullable=False, default=datetime.now(), comment='创建时间')
    gmt_modified = db.Column(db.BigInteger, nullable=False, server_default=text("'0'"), comment='更新时间')
    create_uid = db.Column(db.String(64), nullable=False, server_default=text("'0'"), comment='创建人uid')
    create_uname = db.Column(db.String(64), nullable=False, server_default=text("'0'"), comment='创建人昵称')
    modified_uid = db.Column(db.String(64), nullable=False, server_default=text("'0'"), comment='更新人uid')
    modified_uname = db.Column(db.String(64), nullable=False, server_default=text("'0'"), comment='更新人昵称')
    enable = db.Column(db.SmallInteger, nullable=False, server_default=text("'0'"), comment='是否删除:0-未删除;1-删除')
    merchant_id = db.Column(db.String(32), comment='商户ID')
...
class HomeRecommendProduct(db.Model):
    ...
    sort = db.Column(db.Integer, comment='排序')
    gmt_create = db.Column(db.BigInteger, nullable=False, default=datetime.now(), comment='创建时间')
    gmt_modified = db.Column(db.BigInteger, nullable=False, default=datetime.now(), comment='更新时间')
    create_uid = db.Column(db.String(64), nullable=False, server_default=text("'0'"), comment='创建人uid')
    create_uname = db.Column(db.String(64), nullable=False, server_default=text("'0'"), comment='创建人昵称')
    modified_uid = db.Column(db.String(64), nullable=False, server_default=text("'0'"), comment='更新人uid')
    modified_uname = db.Column(db.String(64), nullable=False, server_default=text("'0'"), comment='更新人昵称')
...
class CmsSubject(db.Model):
    ...
    gmt_create = db.Column(db.BigInteger, nullable=False, server_default=text("'0'"), comment='创建时间')
    gmt_modified = db.Column(db.BigInteger, nullable=False, server_default=text("'0'"), comment='更新时间')
    create_uid = db.Column(db.String(64), nullable=False, server_default=text("'0'"), comment='创建人uid')
    create_uname = db.Column(db.String(64), nullable=False, server_default=text("'0'"), comment='创建人昵称')
    modified_uid = db.Column(db.String(64), nullable=False, server_default=text("'0'"), comment='更新人uid')
